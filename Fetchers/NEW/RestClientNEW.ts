...
export class RestClientNEW {
  ...
  private static parsePageInfoLinks(link: string, query: SearchParams) {
    ...
    for (const link of links) {
      const parsedLink = link.match(RestClientNEW.LINK_HEADER_REGEXP);
      if (!parsedLink) {
        continue;
      }
  ...
  protected async request<T = any>(params: RequestParams) {
    ...
    if (link !== undefined) {
      requestReturn.pageInfo = RestClientNEW.parsePageInfoLinks(link, params.query);
    }
