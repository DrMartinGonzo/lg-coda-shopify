...
interface SendInvoiceArgs extends BaseContext {
  [key: string]: unknown;
  body?: { [key: string]: unknown } | null;
}
interface CompleteArgs extends BaseContext {
  [key: string]: unknown;
...
export class DraftOrder extends Base {
  ...
  public static async find({ session, id, fields = null }: FindArgs): Promise<DraftOrder | null> {
  ...
  public async send_invoice({ body = null, ...otherArgs }: SendInvoiceArgs): Promise<unknown> {
    const response = await this.request<DraftOrder>({
      http_method: 'post',
